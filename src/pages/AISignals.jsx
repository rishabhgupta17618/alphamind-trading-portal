...
export default function AISignals({ creds, trades, onAddTrade }) {
  ...
  // Point 2: Get symbols that already have open trades
  const openTradeSyms = new Set(
    (trades || []).filter(t => t.status === 'OPEN').map(t => t.sym)
  )

  const scan = async () => {
    setL(true); setErr(''); setSignals([]); setAdded({})

    // Exclude stocks already in open trades
    const availableStocks = STOCKS.filter(s => !openTradeSyms.has(s.s))
    const syms = availableStocks.slice(0, parseInt(n)).map(s => s.s)

    if (!syms.length) {
      setErr('All scanned stocks already have open paper trades. Close some trades first or increase stock count.')
      setL(false); return
    }
...
Generate for ALL ${syms.length} stocks: ${syms.join(', ')}`, 2500)
  ...
  return (
    <div className="page">
      <div className="ph">
        <div><div className="pt">AI Signals</div><div className="ps">Scan multiple stocks — already-open trades are automatically excluded</div></div>
      </div>
      ...
      {openTradeSyms.size > 0 && (
        <div className="al al-i">
          <i className="ti ti-info-circle"/>
          <span>
            <b>{openTradeSyms.size} open trade{openTradeSyms.size!==1?'s':''} excluded from scan:</b>{' '}
            {[...openTradeSyms].join(', ')}
          </span>
        </div>
      )}
          ...
          {signals.map(s => {
            ...
            return (
              <div key={s.sym} className="card" style={{borderLeft:`3px solid ${ib?'var(--green)':'var(--red)'}`,borderRadius:'0 14px 14px 0'}}>
                ...
                <button className="btn btn-p btn-sm" disabled={isAdded}
                  onClick={()=>{
                  ...
                  {isAdded ? '✓ Added to paper trades' : 'Add paper trade'}
                </button>
              </div>
            )
          })}
