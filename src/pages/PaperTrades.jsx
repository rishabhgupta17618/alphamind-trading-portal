...
// ─── AUTO-EXIT ENGINE ─────────────────────────────────────────────────────────
// Simulates price movement and auto-exits paper trades at T1 or SL
// Runs as a hook, called from the component that owns trades
export function useAutoExit(trades, closeTrade) {
  useEffect(() => {
    ...
    const timers = open.map(t => {
      ...
      // Swing: 2-10 day hold → random 2–8 min in sim; Intraday: 5–30 min → 30s–2min in sim
      ...
      // 55% chance of hitting T1 (simulating slight positive expectancy from AI signals)
      ...
      const timer = setTimeout(() => {
        closeTrade(t.id, exitPrice, true) // true = auto-exit
      }, delay)
...
function RRCalc({ entry, sl, t1, qty }) {
  ...
  return (
    <div className="g4" style={{marginBottom:10}}>
      ...
      {parseFloat(rr)<1.5 && <div className="al al-w" style={{gridColumn:'1/-1',marginBottom:0}}><i className="ti ti-alert-triangle"/>R:R below 1.5 — consider adjusting levels.</div>}
    </div>
  )
}
        ...
        <div className="card" style={{borderLeft:`3px solid ${result.action==='BUY'?'var(--green)':'var(--red)'}`,borderRadius:'0 14px 14px 0'}}>
          <div style={{display:'flex',justifyContent:'space-between',marginBottom:12}}>
            <div>
              ...
              <span className={`bdg b-${result.action==='BUY'?'buy':'sell'}`} style={{marginLeft:8}}>{result.action}</span>
    ...
    <>
      ...
      <div className="card" style={{border:`1px solid ${D.color}40`}}>
        ...
        <div className="g2">
          ...
          <div className="fr"><label className="fl">Qty</label><input type="number" value={qty} onChange={e=>setQty(+e.target.value)}/></div>
          <div className="fr"><label className="fl">Direction</label>
...
export default function PaperTrades({ creds, trades, onOpen, onClose }) {
  ...
  return (
    <div className="page">
      <div className="ph">
        <div><div className="pt">Paper Trades</div><div className="ps">Simulate trades — auto-exited at T1 or SL · synced across devices</div></div>
