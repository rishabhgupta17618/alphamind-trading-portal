...
:root {
  ...
  --green2: #00a07a;
  --gb: rgba(0,200,150,.1);
  --gbr: rgba(0,200,150,.25);
  --gt: #00c896;
  --red: #ff4d6a;
  --rb: rgba(255,77,106,.1);
  --rbr: rgba(255,77,106,.25);
  --rt: #ff4d6a;
  --amber: #f5a623;
  --ab: rgba(245,166,35,.1);
  --abr: rgba(245,166,35,.25);
  --at: #f5a623;
  --blue: #4d9fff;
  --bb: rgba(77,159,255,.1);
  --bbr: rgba(77,159,255,.25);
  --bt: #4d9fff;
  --purple: #a78bfa;
  --pb: rgba(167,139,250,.1);
  --pbr: rgba(167,139,250,.25);
  --pt: #a78bfa;
}
...
.status-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px; font-size: 11px; color: var(--text3); }
...
.sig-b { background: var(--green); }
.sig-r { background: var(--red); }
.sig-a { background: var(--amber); }
...
.ob-card { width: 100%; max-width: 430px; background: var(--bg2); border: 1px solid var(--border); border-radius: 18px; padding: 30px; }
.cmp th { background: var(--bg3); }
.ch-sw { color: var(--green); }
.ch-id { color: var(--amber); }
.ch-fo { color: var(--purple); }
`
...
// ─── API HELPERS ────────────────────────────────────────────────────────────
async function callClaude(akey, messages, tools = [], max = 2000) {
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-api-key': akey, 'anthropic-version': '2023-06-01' },
    body: JSON.stringify({ model: 'claude-sonnet-4-6', max_tokens: max, messages, ...(tools.length ? { tools } : {}) })
  })
...
async function aiJSON(akey, prompt, max = 2000) {
  const d = await callClaude(akey, [{ role: 'user', content: prompt }], [], max)
  const tx = (d.content?.[0]?.text || '').replace(/```json\n?/g, '').replace(/```\n?/g, '').trim()
  try { return JSON.parse(tx) } catch { return { raw: tx } }
}

async function dhanAPI(cid, tok, akey, ep, method = 'GET', body = null) {
  const url = `https://api.dhan.co/v2${ep}`
  const headers = { 'Content-Type': 'application/json', 'access-token': tok, 'dhanClientId': cid }
  const tools = [{
    name: 'http_request',
  ...
  const d = await callClaude(akey, [{
    role: 'user',
    content: `Make this HTTP request and return the raw JSON response body.
URL: ${url} | Method: ${method} | Headers: ${JSON.stringify(headers)} | Body: ${body ? JSON.stringify(body) : 'null'}
Call the http_request tool now.`
  }], tools, 4096)

  const tu = d.content?.find(x => x.type === 'tool_use')
  if (tu) {
    const r = await fetch(tu.input.url, {
      method: tu.input.method,
      headers: tu.input.headers,
      body: tu.input.body && tu.input.body !== 'null' ? tu.input.body : undefined
    })
    if (!r.ok) { const e = await r.text(); throw new Error(`Dhan ${r.status}: ${e.slice(0, 100)}`) }
    return r.json()
  }
  const tx = d.content?.find(x => x.type === 'text')?.text || ''
  const m = tx.match(/(\{[\s\S]*\}|\[[\s\S]*\])/)
  if (m) try { return JSON.parse(m[0]) } catch {}
  throw new Error('Could not parse Dhan response')
}
...
function Btn({ primary, sm, danger, onClick, disabled, children, style }) {
  return (
    <button
      className={`btn ${primary ? 'btn-p' : ''} ${sm ? 'btn-sm' : ''} ${danger ? 'btn-d' : ''}`}
...
function Badge({ type, children }) {
  return <span className={`bdg b-${type}`}>{children}</span>
}
...
function Settings({ creds, onSave, onboard }) {
  const [c, setC] = useState(creds.cid || '')
  const [t, setT] = useState(creds.tok || '')
  const [a, setA] = useState(creds.akey || '')

  const save = () => {
    if (!c || !t || !a) { alert('Please fill in all three fields.'); return }
    onSave({ cid: c, tok: t, akey: a })
  }

  const form = (
    <>
      <div className="fr"><label className="fl">Dhan Client ID</label>
        <input placeholder="e.g. 1100003626" value={c} onChange={e => setC(e.target.value)} /></div>
      <div className="fr"><label className="fl">Dhan Access Token</label>
        <input type="password" placeholder="eyJ0eXAiOiJKV1Qi…" value={t} onChange={e => setT(e.target.value)} /></div>
      <div className="fr"><label className="fl">Anthropic API Key</label>
        <input type="password" placeholder="sk-ant-api03-…" value={a} onChange={e => setA(e.target.value)} /></div>
    </>
  )
  ...
  if (onboard) return (
    <div className="ob-wrap">
      <div className="ob-card">
        <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--green)', marginBottom: 4 }}>α AlphaMind</div>
        <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 20, letterSpacing: '.5px' }}>AI TRADING PORTAL · DHAN BROKER</div>
        <Alert type="i">Credentials stored only in your browser — sent directly to Dhan and Anthropic APIs, never to any other server.</Alert>
        {form}
        <Btn primary onClick={save} style={{ width: '100%', justifyContent: 'center', marginTop: 8 }}>
          <i className="ti ti-plug" /> Connect &amp; Launch Portal
        </Btn>
        <div style={{ marginTop: 20, fontSize: 12, color: 'var(--text3)', lineHeight: 1.9 }}>
          <b style={{ color: 'var(--text)' }}>Get Dhan credentials:</b><br />
          1. Open <a href="https://web.dhan.co" target="_blank" rel="noreferrer" style={{ color: 'var(--green)' }}>web.dhan.co</a> → My Profile → API Access<br />
          2. Click Generate Access Token → copy Client ID + Token<br /><br />
          <b style={{ color: 'var(--text)' }}>Get Anthropic API Key:</b><br />
          1. Open <a href="https://console.anthropic.com" target="_blank" rel="noreferrer" style={{ color: 'var(--green)' }}>console.anthropic.com</a> → API Keys → Create Key
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="page">
      <div className="ph"><div><div className="pt">Settings</div><div className="ps">API credentials and data management</div></div></div>
      <div className="card">
        <div className="ct"><span><i className="ti ti-key" /> API Credentials</span></div>
        <Alert type="i">Credentials stored in browser localStorage. Sent directly to Dhan and Anthropic — no proxy server involved.</Alert>
        {form}
      ...
      <div className="card">
        <div className="ct">How to get credentials</div>
        <div style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 2 }}>
          <b style={{ color: 'var(--text)' }}>Dhan Client ID + Access Token</b><br />
          1. Log in at <a href="https://web.dhan.co" target="_blank" rel="noreferrer" style={{ color: 'var(--green)' }}>web.dhan.co</a> → My Profile → API Access<br />
          2. Click <b>Generate Access Token</b> (valid 24 h — regenerate daily)<br />
          3. Copy your Client ID and the Access Token<br /><br />
          <b style={{ color: 'var(--text)' }}>Anthropic API Key</b><br />
          1. Go to <a href="https://console.anthropic.com" target="_blank" rel="noreferrer" style={{ color: 'var(--green)' }}>console.anthropic.com</a> → API Keys → Create Key<br />
          2. Copy the key starting with <code style={{ background: 'var(--bg3)', padding: '1px 6px', borderRadius: 4 }}>sk-ant-</code>
        </div>
      </div>
    </div>
  )
...
function Dashboard({ creds, trades, funds, onSyncDhan }) {
  ...
  const open = s.open.slice(0, 5)
  const recent = s.closed.slice(0, 5)

  return (
    <div className="page">
      <div className="ph">
        <div><div className="pt">Dashboard</div>
          <div className="ps">{new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long' })}</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Btn onClick={onSyncDhan}><i className="ti ti-refresh" /> Sync Dhan</Btn>
        </div>
      </div>
      <Alert type="i"><b>Paper Trading Phase</b> — All recommendations are simulated. No real capital at risk. Readiness: <b>{s.rd}/100</b></Alert>
      ...
      <div className="g2">
        ...
        <div className="card">
          <div className="ct"><span><i className="ti ti-flask" /> Open Paper Trades ({open.length})</span></div>
          {open.length === 0
  ...
  const run = async () => {
    ...
    try {
      ...
      try {
        ...
        const h = await dhanAPI(creds.cid, creds.tok, creds.akey, '/charts/historical', 'POST',
      ...
      const a = await aiJSON(creds.akey, `Expert Indian stock analyst. Analyse ${sym} (${tf}).
Last 80 closes: ${closes.slice(-80).join(',')}
Current price: ₹${cmp}
Return ONLY JSON (no markdown):
{"sym":"${sym}","cmp":${cmp},"trend":"Bullish|Bearish|Neutral","strength":"Strong|Moderate|Weak","sma20":<n>,"sma50":<n>,"rsi":<n>,"rsi_sig":"Overbought|Neutral|Oversold","macd":"Bullish|Bearish|Neutral","vol":"High|Normal|Low","s1":<n>,"s2":<n>,"r1":<n>,"r2":<n>,"patterns":["p"],"action":"BUY|SELL|HOLD|AVOID","conviction":"High|Medium|Low","entry_lo":<n>,"entry_hi":<n>,"sl":<n>,"t1":<n>,"t2":<n>,"t3":<n>,"conf":<50-85>,"rr":<n>,"est_profit_100":<n>,"est_loss_100":<n>,"rationale":"3 sentences","trigger":"confirmation","risks":["r1","r2"],"hold":"X days"}`, 2500)
      setResult(a)
  ...
  return (
    <div className="page">
      <div className="ph"><div><div className="pt">Market Analysis</div><div className="ps">AI-powered technical analysis with trade recommendation</div></div></div>
      ...
      {err && <Alert type="e">Analysis failed: {err}. Check your API keys in Settings.</Alert>}
      {!result && !err && !loading && (
        <div style={{ textAlign: 'center', padding: '50px 0', color: 'var(--text3)', fontSize: 12 }}>
          Select a stock and click Run AI Analysis<br />Claude computes RSI, MACD, support/resistance and generates a full trade recommendation
        </div>
      )}
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: 40 }}>
          <i className="ti ti-wand spin" style={{ fontSize: 28, color: 'var(--green)' }} />
          <div style={{ marginTop: 10, color: 'var(--text3)', fontSize: 12 }}>Running AI analysis for <b style={{ color: 'var(--text)' }}>{sym}</b>…</div>
        </div>
      )}
      {result && (
        <div className="card" style={{ borderLeft: `3px solid ${bc}`, borderRadius: '0 14px 14px 0' }}>
          ...
          <div className="g2">
            <div>
              <div style={{ fontSize: 10, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 8 }}>Technical signals</div>
            ...
            <div>
              <div style={{ fontSize: 10, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 8 }}>Support &amp; Resistance</div>
          ...
          <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['swing', 'intraday', 'fno'].map(style => (
              <Btn key={style} primary={style === 'swing'} sm onClick={() => {
                onAddTrade({ sym: result.sym, entry: result.entry_hi, sl: result.sl, t1: result.t1, t2: result.t2 || result.t1, qty: style === 'intraday' ? 100 : style === 'fno' ? 1 : 50, dir: result.action === 'SELL' ? 'SHORT' : 'LONG', style })
              }}>
...
function AISignals({ creds, onAddTrade }) {
  ...
  const scan = async () => {
    ...
    try {
      const r = await aiJSON(creds.akey, `Expert Indian stock analyst. Generate ${style} signals for: ${syms.join(',')}.
Return ONLY JSON array (no markdown). Include only stocks with confidence ≥${minC}%. Mix BUY/SELL/AVOID.
[{"sym":"RELIANCE","cmp":<price>,"action":"BUY|SELL|AVOID","conviction":"High|Medium|Low","entry":<n>,"sl":<n>,"t1":<n>,"t2":<n>,"conf":<n>,"rr":<n>,"rationale":"2 sentence reason","style":"${style}"}]`, 3000)
  ...
  return (
    <div className="page">
      <div className="ph"><div><div className="pt">AI Signals</div><div className="ps">Scan stocks and generate trade recommendations</div></div></div>
      ...
      {signals.length > 0 && (
        <div className="g2">
          {signals.map(s => {
            ...
            const rew = Math.abs(s.t1 - s.entry) * qty
            return (
              <div key={s.sym} className="card" style={{ borderLeft: `3px solid ${ib ? 'var(--green)' : 'var(--red)'}`, borderRadius: '0 14px 14px 0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 3, lineHeight: 1.5 }}>{s.rationale}</div>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                ...
                <div className="g3" style={{ gap: 6, marginBottom: 10 }}>
                  {[['Entry', '₹' + s.entry, 'var(--blue)'], ['Stop loss', '₹' + s.sl, 'var(--red)'], ['Target', '₹' + s.t1, 'var(--green)'],
                    ['T2', '₹' + (s.t2 || s.t1), 'var(--green)'], ['Risk', '₹' + Math.round(risk).toLocaleString('en-IN'), 'var(--red)'], ['Reward', '₹' + Math.round(rew).toLocaleString('en-IN'), 'var(--green)']].map(([l, v, c]) => (
                ...
                <Btn primary sm disabled={added[s.sym]} onClick={() => {
                  ...
                  {added[s.sym] ? '✓ Added' : 'Add paper trade'}
                </Btn>
              </div>
            )
          })}
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        {s.open.length === 0
          ? <div style={{ color: 'var(--text3)', fontSize: 12 }}>No open trades.</div>
          : <div style={{ overflowX: 'auto' }}>
            <table>
              ...
              <tbody>{s.open.map(t => (
                <tr key={t.id}>
                  ...
                  <td><Badge type={t.dir === 'LONG' ? 'buy' : 'sell'}>{t.dir}</Badge></td>
                  <td>₹{t.entry}</td>
                  <td className="dn">₹{t.sl}</td>
                  <td className="up">₹{t.t1}</td>
                  <td>{t.qty}</td>
                  <td>1:{t.rr}</td>
                  <td><Badge type={t.style === 'fno' ? 'fno' : 'paper'}>{t.style || 'swing'}</Badge></td>
                  <td style={{ whiteSpace: 'nowrap' }}>
                    <input type="number" placeholder="Exit ₹" value={exitPrices[t.id] || ''} onChange={e => setExitPrices(p => ({ ...p, [t.id]: e.target.value }))}
                      style={{ width: 72, padding: '4px 7px', fontSize: 11, display: 'inline-block', marginRight: 5 }} />
...
function NewTrade({ onOpen }) {
  ...
  return (
    <div className="card">
      ...
      <div className="g2">
        {[['Symbol', 'sym', 'text', 'e.g. RELIANCE'], ['Quantity', 'qty', 'number', '50']].map(([l, k, t, p]) => (
          <div key={k} className="fr"><label className="fl">{l}</label>
            <input type={t} placeholder={p} value={form[k]} onChange={e => set(k, t === 'number' ? +e.target.value : e.target.value.toUpperCase())} />
          </div>
        ))}
      ...
      <div className="g4" style={{ marginBottom: 12 }}>
        {[['Risk', '₹' + Math.round(risk).toLocaleString('en-IN'), 'dn'], ['Reward', '₹' + Math.round(rew).toLocaleString('en-IN'), 'up'],
          ['R:R', '1:' + rr, parseFloat(rr) >= 1.5 ? 'up' : ''], ['Risk %', ((e && sl ? Math.abs(e - sl) / e * 100 : 0)).toFixed(1) + '%', '']].map(([l, v, cls]) => (
            <div key={l} className="card-sm">
              <div style={{ fontSize: 9, color: 'var(--text3)', textTransform: 'uppercase' }}>{l}</div>
      ...
      {rr > 0 && rr < 1.5 && <Alert type="w">R:R below 1.5 — consider adjusting your levels for a better setup.</Alert>}
      <Btn primary onClick={() => {
...
const STYLE_INFO = {
  swing: { icon: 'ti-trending-up', color: 'var(--green)', name: 'Swing Trading', desc: 'Hold 2–15 days for medium-term price moves. Best for working professionals — only 30-60 min/day required.', defaults: ['RELIANCE', 'LONG', 50, 2840, 2700, 2980, 3050], info: [['Holding period', '2–15 trading days'], ['Min capital', '₹50,000'], ['Recommended capital', '₹2,00,000–₹5,00,000'], ['Risk per trade', '1–2% of capital'], ['Target R:R', '1:2 to 1:3'], ['Monthly return', '3–8%'], ['Max drawdown', '10–20%'], ['Time required', '30–60 min/day (EOD)'], ['Difficulty', '★★☆ Medium'], ['Best suited for', 'Beginners to Advanced']], pros: ['No need to watch screen all day', 'Works alongside a full-time job', 'Large moves clear brokerage costs', 'Lower psychological stress than intraday'], cons: ['Overnight gap risk from news/events', 'Capital locked for multiple days', 'Corporate news can invalidate setups', 'Must hold through temporary pullbacks'] },
  intraday: { icon: 'ti-bolt', color: 'var(--amber)', name: 'Intraday Trading', desc: 'Open and close all positions within the same trading day (9:15 AM – 3:20 PM). Full-time screen attention required.', defaults: ['TCS', 'LONG', 100, 3960, 3920, 4020, 4060], info: [['Holding period', 'Minutes to hours (same day)'], ['Min capital', '₹25,000'], ['Recommended capital', '₹1,00,000–₹3,00,000'], ['Risk per trade', '0.5–1% of capital'], ['Target R:R', '1:1.5 to 1:2'], ['Monthly return', '2–10% (skilled traders)'], ['Max drawdown', '15–30%'], ['Time required', 'Full market hours (5–6 hrs)'], ['Difficulty', '★★★ Hard'], ['Best suited for', 'Intermediate to Advanced ONLY']], pros: ['No overnight risk — flat every evening', '5× leverage on Dhan MIS orders', 'Quick daily feedback loop', 'Profit in both rising and falling markets'], cons: ['Requires constant screen attention', 'Auto square-off at 3:20 PM at market price', 'Very stressful and psychologically taxing', '80%+ beginners lose money — SEBI confirmed'] },
  fno: { icon: 'ti-chart-bar', color: 'var(--purple)', name: 'F&O Trading', desc: 'Trade Nifty/BankNifty futures and index/stock options. Highest leverage, highest complexity, highest risk and reward.', defaults: ['NIFTY FUT', 'LONG', 1, 24200, 24050, 24450, 24700], info: [['Instruments', 'Index & Stock Futures + Options'], ['Nifty lot size', '50 units (≈₹12L+ value)'], ['Futures margin', '₹85,000–₹1,20,000/lot'], ['Options max loss', 'Premium paid only (buyer)'], ['Min capital', '₹1,00,000 (options)'], ['Recommended capital', '₹5,00,000+'], ['Monthly return', '5–20% (huge variance)'], ['Time required', '2–8 hrs/day'], ['Difficulty', '★★★★ Expert'], ['Best suited for', 'Advanced traders ONLY']], pros: ['Control ₹12L+ position with ₹85K margin', 'Options: max loss = premium paid only', 'Weekly expiry creates high-frequency opportunities', 'Can hedge your stock portfolio with puts'], cons: ['Time decay erodes options value daily', 'Most options expire worthless (100% loss)', 'Futures: unlimited loss without strict SL', 'Requires deep understanding of Greeks (Delta/Theta/Vega)', 'SEBI 2023: 90% of F&O traders lose money'] }
}
  ...
  return (
    <>
      ...
      <div className="card" style={{ border: `1px solid ${D.color}40` }}>
        ...
        <div className="g2">
          {[['Symbol', sym, setSym, 'text'], ['Quantity' + (style === 'fno' ? ' (lots)' : ''), qty, setQty, 'number']].map(([l, v, setter, t]) => (
            <div key={l} className="fr"><label className="fl">{l}</label>
              <input type={t} value={v} onChange={e => setter(t === 'number' ? +e.target.value : e.target.value.toUpperCase())} /></div>
          ))}
          <div className="fr"><label className="fl">Direction</label>
          ...
          {[['Entry ₹', entry, setEntry], ['Stop loss ₹', sl, setSl], ['Target 1 ₹', t1, setT1], ['Target 2 ₹', t2, setT2]].map(([l, v, setter]) => (
            <div key={l} className="fr"><label className="fl">{l}</label>
              <input type="number" value={v} onChange={e => setter(+e.target.value)} /></div>
          ))}
        </div>
        <div className="g4" style={{ marginBottom: 12 }}>
          {[['Risk', '₹' + Math.round(risk).toLocaleString('en-IN'), 'dn'], ['Reward', '₹' + Math.round(rew).toLocaleString('en-IN'), 'up'],
            ['R:R', '1:' + rr, parseFloat(rr) >= 1.5 ? 'up' : ''], ['Risk %', ((entry && sl ? Math.abs(entry - sl) / entry * 100 : 0)).toFixed(1) + '%', '']].map(([l, v, cls]) => (
              <div key={l} className="card-sm">
                <div style={{ fontSize: 9, color: 'var(--text3)', textTransform: 'uppercase' }}>{l}</div>
                <div style={{ fontSize: 13, fontWeight: 600 }} className={cls}>{v}</div>
              </div>
...
function Portfolio({ creds }) {
  ...
  const load = async () => {
    ...
    try {
      const [h, p, f] = await Promise.allSettled([
        dhanAPI(creds.cid, creds.tok, creds.akey, '/holdings'),
        dhanAPI(creds.cid, creds.tok, creds.akey, '/positions'),
        dhanAPI(creds.cid, creds.tok, creds.akey, '/fundlimit'),
      ])
  ...
  const aiReview = async () => {
    setReviewing(true)
    try {
      const r = await aiJSON(creds.akey, `Portfolio manager. Review this Indian stock portfolio.
Holdings: ${JSON.stringify(holdings.map(h => ({ sym: h.tradingSymbol, qty: h.totalQty, avg: h.avgCostPrice })))}
Return ONLY JSON: {"summary":"2 sentences","div_score":<1-10>,"risk":"High|Medium|Low","recs":[{"sym":"S","action":"HOLD|ADD|REDUCE|EXIT","reason":"one sentence","urgency":"High|Medium|Low"}],"sector_note":"one sentence","top_risk":"one sentence"}`)
      setReview(r)
    } catch (e) { setErr(e.message) }
    setReviewing(false)
  }
  ...
  return (
    <div className="page">
      <div className="ph">
        <div><div className="pt">Portfolio</div><div className="ps">Live holdings and positions from Dhan</div></div>
        <div style={{ display: 'flex', gap: 8 }}>
      ...
      {funds && (
        <div className="g4" style={{ marginBottom: 14 }}>
          {[['Available balance', Math.round(funds.availabelBalance || 0)], ['SOD limit', Math.round(funds.sodLimit || 0)], ['Utilised', Math.round(funds.utilizedAmount || 0)], ['Withdrawable', Math.round(funds.withdrawableBalance || 0)]].map(([l, v]) => (
      ...
      {holdings.length === 0 && !loading && <div className="card" style={{ textAlign: 'center', padding: 40, color: 'var(--text3)', fontSize: 12 }}>Click "Load from Dhan" to fetch your live holdings and positions.</div>}
      {review && (
...
function Journal({ journal, onAdd }) {
  ...
  return (
    <div className="page">
      <div className="ph">
        <div><div className="pt">Trade Journal</div><div className="ps">{journal.length} entries — trades auto-logged</div></div>
      </div>
      <div className="card">
        <div className="ct"><i className="ti ti-edit" /> Add note</div>
        <div style={{ display: 'flex', gap: 8 }}>
          <input placeholder="Market observation, lesson learned, trade note…" value={note} onChange={e => setNote(e.target.value)} style={{ flex: 1 }} />
          <Btn primary onClick={() => { if (!note.trim()) return; onAdd({ type: 'note', note }); setNote('') }}>
...
function StrategyGuide() {
  ...
  return (
    <div className="page">
      ...
      <div className="card">
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="cmp">
            ...
            <tbody>{rows.map(([p, s, i, f], idx) => (
              <tr key={p} style={{ background: idx % 2 === 0 ? 'var(--bg3)' : 'transparent' }}>
                <td style={{ color: 'var(--text3)' }}>{p}</td>
                <td className="ch-sw">{s}</td><td className="ch-id">{i}</td><td className="ch-fo">{f}</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
      ...
      <Alert type="w"><div><b>Honest reality check:</b> 80% of intraday traders and 90% of F&O traders lose money (SEBI 2023). Profitable trading requires a tested edge, strict risk management, and iron discipline — not just analysis tools. This paper trading phase exists to build and test that edge before risking real capital.</div></Alert>
    </div>
  )
}
...
export default function App() {
  ...
  const openTrade = useCallback((t) => {
    ...
    const trade = { id, ...t, status: 'OPEN', pnl: null, result: null, openedAt: new Date().toISOString(), closedAt: null, rr: risk > 0 ? (rew / risk).toFixed(2) : '—', risk: Math.round(risk), reward: Math.round(rew) }
  ...
  const closeTrade = useCallback((id, exitP) => {
    setTrades(prev => prev.map(t => {
      ...
      setJournal(prev => [{ id: 'J' + Date.now().toString(36), ts: new Date().toISOString(), type: pnl > 0 ? 'win' : 'loss', sym: t.sym, pnl: closed.pnl, note: `Closed @ ₹${exitP} — ${t.dir}` }, ...prev])
      return closed
    }))
  }, [setTrades, setJournal])
  ...
  const syncDhan = useCallback(async () => {
    try {
      const f = await dhanAPI(creds.cid, creds.tok, creds.akey, '/fundlimit')
      setFunds(f)
  ...
  const groups = ['Main', 'Trading', 'Learn']
  const s = calcStats(trades)
        ...
        <nav className="sidebar">
          ...
          <div className="sb-foot">
            <div className="status-row">
              <span><span className="dot live" />Dhan: ready</span>
            </div>
            <div className="status-row">
              <span><span className="dot blue" /><span style={{ color: 'var(--blue)', fontSize: 11 }}>Paper phase · {s.rd}/100</span></span>
            </div>
